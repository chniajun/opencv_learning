...
bool MultiChannelBlending()
{
       ...
       imageBlueChannel=channels.at(0); // BGR
       //【4】将原图的蓝色通道的（50,25）坐标处右下方的一块区域和logo图进行加权操作，将得到的混合结果存到imageBlueChannel中
       addWeighted(imageBlueChannel(Rect(50,25,logoImage.cols,logoImage.rows)),1.0,
              logoImage,0.5,0,imageBlueChannel(Rect(50,25,logoImage.cols,logoImage.rows))); // 类似于ROI
       ...
       //【4】将原图的绿色通道的（50,25）坐标处右下方的一块区域和logo图进行加权操作，将得到的混合结果存到imageGreenChannel中
       addWeighted(imageGreenChannel(Rect(50,25,logoImage.cols,logoImage.rows)),1,
       ...
       //【4】将原图的红色通道的（50,25）坐标处右下方的一块区域和logo图进行加权操作，将得到的混合结果存到imageRedChannel中
